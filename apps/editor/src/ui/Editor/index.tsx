...
export const Editor: FC<{}> = () => {
  const {user, lastCommitBody, chapter} = useLoaderData() as {
    user: UserJSON
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        style: 'min-height: calc(-64px + 100vh);',
        class:
  ...
  return (
    <>
      <div className="border-b border-gray-200 mb-5 pb-5">
        <h3 className="text-base font-semibold leading-6 text-gray-900">{chapter.title}</h3>
      </div>
