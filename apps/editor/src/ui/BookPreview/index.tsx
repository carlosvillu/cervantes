...
export const BookPreview: FC<Props> = ({book, chapter, body, links}) => {
  ...
  const handleLinkClick = (chapterId: String) => {
    setIsVisible(false)
    setTimeout(() => {
      const url = `/book/${bookId}/preview/${String(chapterId)}`
      navigate(url)
      setIsVisible(true)
    }, TRANSITION_DURATION)
  }
    ...
    >
      ...
      >
        ...
        <div className="flex flex-1 relative items-center justify-center	">
          ...
          <div
            className="z-10 relative bg-slate-100/40 p-4 my-6 mx-2 h-fit rounded-lg text-center text-pretty backdrop-blur-sm text-2xl"
        ...
        <div className="bg-gray-800/80 z-10">
          <ul className="flex flex-col justify-around items-center p-2 cursor-pointer">
            ...
            {!links.length && (
              <li
                className="m-2 pointer bg-slate-100/10 text-slate-100/80  px-4 py-2 rounded-lg hover:bg-slate-100/20 hover:text-slate-100 transition-all duration-200 ease-in-out w-full text-center text-2xl backdrop-blur-sm"
                onClick={() => handleLinkClick(MOCKED_ROOT_CHAPTER)}
              >
                Volver a emperzar
              </li>
            )}
